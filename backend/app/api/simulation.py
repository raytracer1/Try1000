...
def start_simulation(req: SimulateRequest, user_id: str = Depends(get_current_user),
    ...
    job = SimulationJob(
        ...
        home_tactic_id=req.home_tactic_id,
        away_tactic_id=req.away_tactic_id,
    ...
    thread = threading.Thread(
        target=run_simulation_job, args=(str(job.id),), daemon=True
    )
    thread.start()

    return SimulateResponse(job_id=str(job.id))


@router.get("/simulation/jobs", response_model=list[JobStatusResponse])
def list_jobs(user_id: str = Depends(get_current_user), db: Session = Depends(get_db)):
...
def get_replay(job_id: str, match_index: int,
    ...
    result = db.query(SimulationResult).filter(
        SimulationResult.job_id == job_id,
        SimulationResult.match_index == match_index,
    ).first()
    if not result:
        raise HTTPException(status_code=404)
...
def _job_to_status(j: SimulationJob) -> JobStatusResponse:
    return JobStatusResponse(
        ...
        home_team_id=str(j.home_team_id), away_team_id=str(j.away_team_id),
