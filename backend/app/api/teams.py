...
def list_teams(user_id: str = Depends(get_current_user), db: Session = Depends(get_db)):
    teams = db.query(Team).filter(Team.user_id == user_id).all()
    return [_team_to_response(t) for t in teams]


@router.post("", response_model=TeamResponse)
def create_team(req: TeamCreate, user_id: str = Depends(get_current_user),
...
def add_player(team_id: str, req: PlayerCreate,
...
def delete_player(player_id: str, user_id: str = Depends(get_current_user),
    ...
    player = db.query(Player).join(Team).filter(
        Player.id == player_id, Team.user_id == user_id).first()
    if not player:
        raise HTTPException(status_code=404)
    db.delete(player)
    db.commit()
...
def _team_to_response(team: Team) -> TeamResponse:
    return TeamResponse(
        id=str(team.id), user_id=str(team.user_id), name=team.name,
        players=[_player_to_response(p) for p in (team.players or [])],
    )


def _player_to_response(p: Player) -> PlayerResponse:
    return PlayerResponse(
        id=str(p.id), name=p.name, number=p.number,
